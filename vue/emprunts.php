...
<html lang="fr" font-color=#FFFFF>
	...
	<body>
		<section class="header"  style="background-color:rgba(4,9,30,0.35)">
            ...
            <h1>Les emprunts</h1>
                <?php
                $emprunts = ControleurAdmin::listeEmprunts();
                if(empty($users)) {
                ?>
                <p>Il n'y a pas encore d'emprunt inscrit.</p>
                <?php } else { ?>
                ...
                <table  width="80%" valign="center" align="center" bg-color="#FFFFFF">
                    ...
                    <?php foreach($emprunts as $e) { ?>
                        <tr>
                            <?php
                            $login = $e->getLogin();
                            $exemp = $e->getExemplaire();
                            $dateEmprunt = $e->getDate_emp();
                            $rendu = (is_null($e->getDate_ret())) ? "Non rendu" : "Rendu";
                            echo "
                            <td>$login</td>
                            <td>$exemp</td>
                            <td style='text-align:left'>$dateEmprunt</td>
                            <td style='text-align:left'>$rendu</td>
                            "
                            ?>
                        </tr>
                    <?php }?>
